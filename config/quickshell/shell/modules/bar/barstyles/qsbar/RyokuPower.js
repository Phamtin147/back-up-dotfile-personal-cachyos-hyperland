...
var backlightDeviceCmd = "BL=$(ls -1 /sys/class/backlight 2>/dev/null | head -n1); for C in /sys/class/backlight/gmux_backlight /sys/class/backlight/apple_gmux /sys/class/backlight/amdgpu_bl* /sys/class/backlight/intel_backlight /sys/class/backlight/acpi_video*; do [ -e \"$C\" ] && { BL=\"${C##*/}\"; break; }; done; [ -n \"$BL\" ] || { for C in /sys/class/backlight/*; do [ -e \"$C\" ] && [ \"${C##*/}\" != \"appletb_backlight\" ] && { BL=\"${C##*/}\"; break; }; done; }; [ -n \"$BL\" ] || exit 0; "
...
function brightnessSetCmd(step) {
    ...
    return "RUNTIME_DIR=${XDG_RUNTIME_DIR:-/tmp}; exec 9>\"$RUNTIME_DIR/ryoku-qsbar-brightness.lock\"; flock -n 9 || exit 0; " +
        ...
        "PCT=$(brightnessctl -d \"$BL\" -m 2>/dev/null | cut -d',' -f4 | tr -d '%' | awk '{print int($1)}'); " +
...
var batteryDataCmd =
    ...
    "AC=$(cat /sys/class/power_supply/ADP*/online /sys/class/power_supply/AC*/online 2>/dev/null | head -n1 || echo 0); " +
    "if [ \"$AC\" = '1' ] && [ \"$STATE\" = 'discharging' ]; then STATE='plugged-in'; fi; " +
    "LABEL=$(case \"$STATE\" in charging) echo 'Time to full';; plugged-in) echo 'Plugged in';; *) echo 'Time left';; esac); " +
